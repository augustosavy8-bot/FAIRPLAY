...
// ── Fallbacks (usados solo si el server no devuelve datos) ────
...
// ── localStorage cache (acelera re-visitas SPA) ───────────────
...
export default function StoreClient({
  ...
  // Estado — se prefiere dato del server; si no hay, se lee cache; si no, fallback
  const [products,    setProducts]    = useState(() => initialProducts.length    ? initialProducts    : (readCache(CACHE_PRODUCTOS, TTL_SHORT) || []));
  const [heros,       setHeros]       = useState(() => initialHeros.length       ? initialHeros       : (readCache(CACHE_HEROS,     TTL_LONG)  || [FALLBACK_HERO]));
  const [cats,        setCats]        = useState(() => initialCats.length        ? initialCats        : (readCache(CACHE_CATS,      TTL_LONG)  || FALLBACK_CATS));
  ...
  // Con datos del server ya están listos; sin datos usa cache o espera 2s
  const [productsLoaded, setProductsLoaded] = useState(
    () => initialProducts.length > 0 || !!(readCache(CACHE_PRODUCTOS, TTL_SHORT)?.length)
  );
  ...
  useEffect(() => {
    if (initialProducts.length)    writeCache(CACHE_PRODUCTOS, initialProducts);
    if (initialHeros.length)       writeCache(CACHE_HEROS,     initialHeros);
    if (initialCats.length)        writeCache(CACHE_CATS,      initialCats);
    if (initialBannerCards.length) writeCache(CACHE_BANNERS,   initialBannerCards);
    // Skeleton nunca dura más de 2s si el server no envió productos
    if (!initialProducts.length) {
      const t = setTimeout(() => setProductsLoaded(true), 2000);
      ...
      <footer style={{ background:'#0a0a0a',padding:'32px clamp(16px,4vw,48px) 24px',borderTop:'1px solid rgba(255,255,255,.05)' }}>
        <div style={{ maxWidth:1400,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:14 }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:8 }}>
            ...
            <img src="/LOGO_FOKO.png" alt="Foko" style={{ height:26,width:'auto',opacity:.7 }} />
          </div>
        </div>
      </footer>
